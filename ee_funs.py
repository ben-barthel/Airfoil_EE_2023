...
def guth_criterion2(a,b,nq=50,nb=101,q_min=0,q_max=0.2,Q=None):
    """
    Computes the criterion descibed by equation 17 in Guth and Sapsis, Entropy, 2019
    Max is taken over a grid of nq values of q (extreme event rate) from 1/nb to q_max
    Inputs:
...
def optfun(x,a,g):
    ...
    b = np.matmul(g,x)
    alpha = guth_criterion2(a,b,nq=51,nb=501,q_min=0,q_max=0.5,Q=None)
    alpha = -alpha
    #alpha = np.absolute(1 - np.matmul(np.transpose(x),x))
    print(alpha)
    return alpha
